...
public class GPS extends TestActivity implements OnClickListener {
    ...
    private static class MyHandler extends Handler {
        private WeakReference<GPS> reference;

        public MyHandler(GPS activity) {
            reference = new WeakReference<GPS>(activity);//这里传入activity的上下文
        }
        @Override
        public void handleMessage(Message msg) {
            GPS activity = reference.get();
            switch (msg.what) {
                case 0:
                    activity.count++;
